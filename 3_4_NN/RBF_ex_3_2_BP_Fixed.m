...
% File name: RBF_ex_3_2_BP_fixed.m
...
%% Ex_3_2 - Levenberg-Madquardt Learning Algorithm for RBF Neural net
...
%% LM:
% % -----------------------------------------------------------------------
% % -- ENTIRE DATA BATCH TRAINING ALGORITHM --> SIMILAR TO BATCH GRADIENT DESCENT
% % -----------------------------------------------------------------------
...
cost_BP(1,1) = (1/2)*sum((Cm_train - output_train.Y2).^2); % training cost
cost_BP(1,2) = (1/2)*sum((Cm_val - output_val.Y2).^2); % validation cost
cost_BP(1,3) = (1/2)*sum((Cm_test - output_test.Y2).^2); % testing cost
...
for i = 1:N_epochs
    ...
    cost_BP(i+1,1) = (1/2)*sum((Cm_train - output_train.Y2).^2); 
    % - Compute validation cost:
    cost_BP(i+1,2) = (1/2)*sum((Cm_val - output_val.Y2).^2);
    % - Compute testing cost:
    cost_BP(i+1,3) = (1/2)*sum((Cm_test - output_test.Y2).^2);
    ...
    if cost_BP(i+1,1) < cost_BP(i,1)
        % Keep the changes: 
        net = net_aux; 
    elseif cost_BP(i+1,1) >= cost_BP(i,1) 
...
plot_Cm(Cm,net_output.Y2,Z_k_rec(:,[1 2]),num_fig,['RBF-NN - ' data_set ' -> Reconstructed C_m for ' num2str(HN) ' neurons']);
num_fig = num_fig + 1;
plot_RBF_inputs(Cm,num_fig,['RBF-NN - ' data_set ' -> For ' num2str(HN) ' neurons'],Z_k_rec(:,[1 2]),net.centers,input_train',input_val',input_test',Cm_train',Cm_val',Cm_test');