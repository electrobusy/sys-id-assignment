...
cost_BP(1,1) = (1/(2*length(y_train)))*sum((y_train - output_train.Y2).^2); % training cost
cost_BP(1,2) = (1/(2*length(y_val)))*sum((y_val - output_val.Y2).^2); % testing cost
cost_BP(1,3) = (1/(2*length(y_test)))*sum((y_test - output_test.Y2).^2); % testing cost
...
for i = 1:N_epochs 
    % - Forward prop:
    % output_train = simNet(net_aux,input_train,net.name); % no need
    ...
    cost_BP(i+1,1) = (1/(2*length(y_train)))*sum((y_train - output_train.Y2).^2); 
    % - Compute training cost:
    cost_BP(i+1,2) = (1/(2*length(y_val)))*sum((y_val - output_val.Y2).^2);
    % - Compute testing cost:
    cost_BP(i+1,3) = (1/(2*length(y_test)))*sum((y_test - output_test.Y2).^2);
    ...
    if cost_BP(i+1,1) < net_aux.trainParam.goal || abs(min(grad)) < net_aux.trainParam.min_grad
        % cost_BP(i+1,:) = cost_BP(i,:);
        break;
    end
